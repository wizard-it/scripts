...
class FileLock(object):
    ...
    def valid_lock(self):
        ...
        my_pid = os.getpid()
        lock_pid = open(self.lockfname).read()
        # print(lock_pid)
...
# TestProcessKilled.test_force_unlock_in_same_process('test.txt')
timeout = 10
lock = FileLock('test.txt', timeout=timeout, force=False)
with lock:
    time.sleep(10)
#
# lockfile = 'test.txt.lock'
...
#     f.write(str(os.getpid()))
# timeout = 10
# lock = FileLock('test.txt', timeout=timeout, force=True)
# print(str(os.getpid()))
# with lock:
#     print('t')
